...
from Switch.TTP_TEMPLATES.config_ar2530 import *
...
def conectar_sw(ip):
    ...
    if response != 0:
        ...
        erro_zabbix = "zabbix_sender -z ==ENDERECO_IP_ZABBIX== -s Projeto_Switch_Netbox -k Trap_Erros_Atualizar_Configuracao -o \"" + ip + " - LOGIN - OFFLINE\""
    ...
    try:
        ...
        erro_zabbix = "zabbix_sender -z ==ENDERECO_IP_ZABBIX== -s Projeto_Switch_Netbox -k Trap_Erros_Atualizar_Configuracao -o \"" + ip + " - LOGIN  - ERRO AO CONECTAR - TIMEOUT\""
        os.system(erro_zabbix)
        return retorno
        ...
        erro_zabbix = "zabbix_sender -z ==ENDERECO_IP_ZABBIX== -s Projeto_Switch_Netbox -k Trap_Erros_Atualizar_Configuracao -o \"" + ip + " - LOGIN  - ERRO AO CONECTAR - AUTENTICACAO\""
        os.system(erro_zabbix)
        return retorno
    except Exception:
        ...
        erro_zabbix = "zabbix_sender -z ==ENDERECO_IP_ZABBIX== -s Projeto_Switch_Netbox -k Trap_Erros_Atualizar_Configuracao -o \"" + ip + " - LOGIN  - ERRO AO CONECTAR - DIVERSOS\""
        os.system(erro_zabbix)
        return retorno
    # Coleta o prompt do ativo
    prompt = net_connect.find_prompt()
    # Verificar o status do STP
    #retorno += ttp_show_spanning_tree(net_connect, ip, log_exe, log_data)
    update_stp(net_connect, ip, log_exe, log_data)
    return retorno
...
def main(x):
    ...
    print('PROGRAMA PARA VERIFICAR O STATUS DO SPANNING TREE NOS ATIVOS ARUBA 2530')
    ...
    try:
        if ipaddress.ip_address(x) != True:
            print('NÃO É PERMITIDO REALIZAR ESSA FUNÇÃO POR IP')
            # print('IP VALIDADO')
            # ip = x
            # var_controle = 10
    except Exception:
    ...
    # Lista todos os sites cadastrados no CMDB
        ...
        for site in sites:
            ...
            if site['id'] == 206 or site['id'] == 108:
                break
            print(f"Iniciando a verificação do STP nos ativos Aruba 2530 do site: {site['name']}, ID: {site['id']}")
            print('=' * 60)
    ...
    if var_controle == 10:
        print(f"Iniciando a verificação do STP no ativo: {ip}")
        conectar_sw(ip)
    if var_controle == 1:
        # Lista todos os sites cadastrados no CMDB
        sites = cmdb_rest.list_all_sites()
...
logfile_exec = log_dir + '/UPDATE_Aruba_2530_validar_stp_' + str(hora.strftime("%Y-%m-%d") + '.csv')
logfile_dataset = log_dir + '/DATASETS/UPDATE_Ar_2530_stp.csv'
...
if not os.path.exists(logfile_dataset):
    with open(logfile_dataset, mode='w', newline='') as file:
        file.write("DATA,HORA,STATUS,MENSAGEM,COMPLEMENTO,IP\n")
        file.close()
