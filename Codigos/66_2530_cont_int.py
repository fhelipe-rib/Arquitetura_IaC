...
def conectar_sw(ip):
    ...
    if response != 0:
        ...
        erro_zabbix = "zabbix_sender -z ==ENDERECO_IP_ZABBIX== -s Projeto_Switch_Netbox -k Trap_Erros_Contadores_Interfaces -o \"" + ip + " - LOGIN - OFFLINE\""
    ...
    try:
        ...
        erro_zabbix = "zabbix_sender -z ==ENDERECO_IP_ZABBIX== -s Projeto_Switch_Netbox -k Trap_Erros_Contadores_Interfaces -o \"" + ip + " - LOGIN  - ERRO AO CONECTAR - TIMEOUT\""
        os.system(erro_zabbix)
        return retorno
        ...
        erro_zabbix = "zabbix_sender -z ==ENDERECO_IP_ZABBIX== -s Projeto_Switch_Netbox -k Trap_Erros_Contadores_Interfaces -o \"" + ip + " - LOGIN  - ERRO AO CONECTAR - AUTENTICACAO\""
        os.system(erro_zabbix)
        return retorno
    except Exception:
        ...
        erro_zabbix = "zabbix_sender -z ==ENDERECO_IP_ZABBIX== -s Projeto_Switch_Netbox -k Trap_Erros_Contadores_Interfaces -o \"" + ip + " - LOGIN  - ERRO AO CONECTAR - DIVERSOS\""
        os.system(erro_zabbix)
        return retorno
    # Coleta o prompt do ativo
    prompt = net_connect.find_prompt()
    # Verificar o status do STP
    retorno += verificar_contadores_interfaces(net_connect, ip, log_exe, log_data)
    return retorno
...
def main(x):
    ...
    print('PROGRAMA PARA VERIFICAR OS CONTADORES DAS INTERFACES NOS ATIVOS ARUBA 2530')
    ...
    # Lista todos os sites cadastrados no CMDB
        ...
        for site in sites:
            ...
            if site['id'] == 206 or site['id'] == 108:
                break
            print(f"Iniciando a verificação dos contadores das interfaces nos ativos Aruba 2530 do site: {site['name']}, ID: {site['id']}")
            print('=' * 60)
...
logfile_exec = log_dir + '/Aruba_2530_verif_contadores_interf_' + str(hora.strftime("%Y-%m-%d") + '.csv')
logfile_dataset = log_dir + '/DATASETS/Ar_2530_contadores_interf.csv'
...
if not os.path.exists(logfile_dataset):
    with open(logfile_dataset, mode='w', newline='') as file:
        file.write("DATA,HORA,STATUS,INTERFACE,MENSAGEM,VALOR,IP\n")
        file.close()
