...
def conectar_sw(ip, vizinhos_cmdb, ativo_sysn):
    ...
    if response != 0:
        ...
        erro_zabbix = "zabbix_sender -z ==ENDERECO_IP_ZABBIX== -s Projeto_Switch_Netbox -k Trap_Erros_Estado_Enlaces -o \"" + ip + " - LOGIN - OFFLINE\""
    ...
    try:
        ...
        erro_zabbix = "zabbix_sender -z ==ENDERECO_IP_ZABBIX== -s Projeto_Switch_Netbox -k Trap_Erros_Estado_Enlaces -o \"" + ip + " - LOGIN  - ERRO AO CONECTAR - TIMEOUT\""
        os.system(erro_zabbix)
        return retorno
        ...
        erro_zabbix = "zabbix_sender -z ==ENDERECO_IP_ZABBIX== -s Projeto_Switch_Netbox -k Trap_Erros_Estado_Enlaces -o \"" + ip + " - LOGIN  - ERRO AO CONECTAR - AUTENTICACAO\""
        os.system(erro_zabbix)
        return retorno
    except Exception:
        ...
        erro_zabbix = "zabbix_sender -z ==ENDERECO_IP_ZABBIX== -s Projeto_Switch_Netbox -k Trap_Erros_Estado_Enlaces -o \"" + ip + " - LOGIN  - ERRO AO CONECTAR - DIVERSOS\""
        os.system(erro_zabbix)
        return retorno
    # Coleta o prompt do ativo
    prompt = net_connect.find_prompt()
    # Verificar o status do STP
    retorno += validar_links_ativos(net_connect, ip, vizinhos_cmdb, ativo_sysn, log_exe, log_data)
    return retorno
...
def valida_ativo(dev):
    if dev['device_role']['id'] == 4 or dev['device_role']['id'] == 5 or dev['device_role']['id'] == 6 or dev['device_role']['id'] == 7:
        ...
        if dev['device_type']['id'] == 25 or dev['device_type']['id'] == 26 or dev['device_type']['id'] == 27 or dev['device_type']['id'] == 96:
            # Consulta o CMDB e coleta os devices vizinhos
            cabos_conectados = cmdb_rest.list_cabos_por_device(dev['id'])
            # Consulta nome do ativo
            ativo_sysname = dev['name']
            # Conecta ao equipamento
            retorno_valida = dev['primary_ip4']['address'][:-3]
            retorno_valida += '\n' + dev['name'] + '\n'
            retorno_valida += conectar_sw(dev['primary_ip4']['address'][:-3], cabos_conectados, ativo_sysname)
            retorno_valida += '\n' + '-' * 40
...
def main(x):
    ...
    print('PROGRAMA PARA VALIDAR O STATUS DO ENLACES DOS ATIVOS ARUBA 2530')
    ...
    # Lista todos os sites cadastrados no CMDB
        ...
        for site in sites:
            ...
            if site['id'] == 206 or site['id'] == 108:
                break
            print(f"Iniciando a verificação dos enlaces nos ativos Aruba 2530 do site: {site['name']}, ID: {site['id']}")
            print('=' * 60)
...
logfile_exec = log_dir + '/Aruba_2530_validar_enlaces_' + str(hora.strftime("%Y-%m-%d") + '.csv')
logfile_dataset = log_dir + '/DATASETS/Ar_2530_validar_enlaces.csv'
...
if not os.path.exists(logfile_dataset):
    with open(logfile_dataset, mode='w', newline='') as file:
        file.write("DATA,HORA,STATUS,MENSAGEM,INTERFACE,IP\n")
        file.close()
