...
def conectar_sw(ip, vizinhos_cmdb, ativo_sysn):
    ...
    if response != 0:
        ...
        erro_zabbix = "zabbix_sender -z ==ENDERECO_IP_ZABBIX== -s Projeto_Switch_Netbox -k Trap_Erros_Vizinhos -o \"" + ip + " - LOGIN - OFFLINE\""
    ...
    try:
        ...
        erro_zabbix = "zabbix_sender -z ==ENDERECO_IP_ZABBIX== -s Projeto_Switch_Netbox -k Trap_Erros_Vizinhos -o \"" + ip + " - LOGIN  - ERRO AO CONECTAR - TIMEOUT\""
        os.system(erro_zabbix)
        return retorno
    except NetMikoAuthenticationException:
        retorno += ip + " - ERRO AO CONECTAR - AUTENTICACAO"
        logging.warning(f"FALHA,ERRO AO CONECTAR - AUTENTICACAO,,,{ip}")
        erro_zabbix = "zabbix_sender -z ==ENDERECO_IP_ZABBIX== -s Projeto_Switch_Netbox -k Trap_Erros_Vizinhos -o \"" + ip + " - LOGIN  - ERRO AO CONECTAR - AUTENTICACAO\""
        os.system(erro_zabbix)
        return retorno
    except Exception:
        retorno += ip + " - ERRO AO CONECTAR - DIVERSOS"
        logging.warning(f"FALHA,ERRO AO CONECTAR - DIVERSOS,,,{ip}")
        erro_zabbix = "zabbix_sender -z ==ENDERECO_IP_ZABBIX== -s Projeto_Switch_Netbox -k Trap_Erros_Vizinhos -o \"" + ip + " - LOGIN  - ERRO AO CONECTAR - DIVERSOS\""
        os.system(erro_zabbix)
        return retorno
    # Coleta o prompt do ativo
    prompt = net_connect.find_prompt()
    # Verificar os enlaces entre os ativos vizinhos
    retorno += validar_vizinhos_lldp(net_connect, ip, vizinhos_cmdb, ativo_sysn)
    return retorno
...
def main(x):
    ...
    print('PROGRAMA PARA VALIDAR OS VIZINHOS LLDP COM O CMDB NOS ATIVOS ARUBA 2530')
    ...
    logfile = log_dir + '/' + 'Aruba_2530_verif_vizinhos_' + str(hora.strftime("%Y-%m-%d") + '.csv')
    ...
    if not os.path.exists(logfile):
        with open(logfile, mode='w', newline='') as file:
            file.write("DATA,HORA,STATUS,MENSAGEM,CMDB,LLDP,IP\n")
            file.close()
    # Configuração do log
    ...
    # Lista todos os sites cadastrados no CMDB
        ...
        for site in sites:
            ...
            if site['id'] == 206 or site['id'] == 108:
                break
            print(f"Iniciando a verificação dos vizinhos nos ativos Aruba 2530 do site: {site['name']}, ID: {site['id']}")
            print('=' * 60)
